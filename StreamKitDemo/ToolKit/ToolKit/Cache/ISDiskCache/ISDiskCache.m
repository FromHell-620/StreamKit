...
-(void)setData:(NSData *)data forKey:(id<NSCoding>)key
{
    if (data ==nil || key == nil) {
        return;
    }
...
-(NSData *)dataForKey:(id <NSCoding>)key
{
    if (![self hasObjectForKey:key]) {
        return nil;
    }
...
- (void)saveWithFilePath:(NSString *)srcFilePath forKey:(id <NSCoding>)key
{
    dispatch_semaphore_wait(self.semaphore, DISPATCH_TIME_FOREVER);
    NSString *path = [self filePathForKey:key];
    NSString *directoryPath = [path stringByDeletingLastPathComponent];
    ...
    NSError *error = nil;
    [fileManager moveItemAtPath:srcFilePath toPath:path error:&error];
